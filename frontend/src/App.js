...
function App() {
  ...
  useEffect(()=>{
    if(location.pathname=="/uploads"){
      ...
      document.querySelector(".uploads").classList.add("hover:bg-[#69a6f0]");
      ...
      document.querySelector(".sharedWithYou").classList.add("hover:bg-[#69a6f0]");
      ...
      document.querySelector(".sharedByYou").classList.add("hover:bg-[#69a6f0]");
    
    }else if(location.pathname=="/"){
      document.querySelector(".home").classList.add("bg-blue-500");
      document.querySelector(".home").classList.add("text-white");
      document.querySelector(".home").classList.add("hover:bg-[#69a6f0]");
  ...
  <nav className="w-full h-14 bg-slate-400 flex justify-evenly py-2">
  <FaBars className="h-10 w-10 lg:hidden p-2" onClick={handleMenu}/>

 
    <div className="logo h-10 w-40  flex justify-center items-center text-lg font-bold lg:text-2xl">
      <a href="/"><p>file-in-touch</p></a>
    </div>
    
    <div className="flex w-[40%] mr-5 lg:mr-0">
      ...
      {(showLogin)?<div className='flex flex-col lg:flex-row'>
      <p className='underline my-auto cursor-pointer text-xl' onClick={()=>setRegisterModel(true)}>register</p>
      <p className='my-auto mx-3'>  /  </p>
      <button className=' font-bold h-15 w-20 p-2 text-xl text-center rounded-2xl bg-yellow-500' onClick={()=>setLoginModal(true)}>login</button>
      </div>:<></>}
   
    <div className="h-10 w-10 flex flex-col relative p-1 mr-2 lg:p-0">
      {/* <img
  ...
  <path d="M144 480C64.5 480 0 415.5 0 336c0-62.8 40.2-116.2 96.2-135.9c-.1-2.7-.2-5.4-.2-8.1c0-88.4 71.6-160 160-160c59.3 0 111 32.2 138.7 80.2C409.9 102 428.3 96 448 96c53 0 96 43 96 96c0 12.2-2.3 23.8-6.4 34.6C596 238.4 640 290.1 640 352c0 70.7-57.3 128-128 128H144zm79-217c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0l39-39V392c0 13.3 10.7 24 24 24s24-10.7 24-24V257.9l39 39c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-80-80c-9.4-9.4-24.6-9.4-33.9 0l-80 80z" />
      ...
      <form action="" className='hidden' onSubmit={submitForm} >
        ...
        <a href="/" element={<Home/>} className="home h-10 w-[85%]  mx-auto rounded-3xl text-center hover:bg-slate-200 flex justify-center items-center cursor-pointer" >
          Home
        </a>
      
      <hr className="border-1 border-slate-600 border-dashed mx-5" />
      
      <a exact href="/uploads" element={<UploadedByMe />} className="uploads text-black focus:text-red-600 h-10 w-[85%]  mx-auto rounded-3xl text-center hover:bg-slate-200 flex justify-center items-center cursor-pointer ">
          Uploaded By You
        </a>
    ...
    {/* flex flex-wrap gap-10  justify-start items-start */}
      ...
      {/* <UploadedByMe uploadedData={uploadedData}/> */}
          <Routes>
            ...
            <Route exact path='/uploads' element={<UploadedByMe setLoginModal={setLoginModal} setShowLogin={setShowLogin}/>} onChange={()=>setUpdater(prev=>!prev)}/>
