...
function Login({setLogin,setUserId,setRegisterModel}) {
    const modalRef= useRef();
    const [email,setEmail] = useState();
    const [password,setPassword] = useState();

    const closeModel=(e)=>{
        if(modalRef.current===e.target)  setLogin(false);
    }
    const submitForm=async(e)=>{
        ...
        const config = {
          ...
          let data;
        try {
          data=await axios.post(`${window.location.origin}/api/login`,{email,password},config);
        } catch (error) {
          setLogin(false);
  ...
  return (
    <div ref={modalRef} className='flex fixed inset-0 bg-black bg-opacity-10 backdrop-blur-sm justify-center items-center' onClick={closeModel}>
            ...
            <ImCross className='absolute right-0 -top-5' onClick={()=>setLogin(false)}/>
                <form className='flex flex-col p-10 justify-evenly' onSubmit={submitForm}>
                    ...
                    <button type="submit" className="text-white bg-purple-700 hover:bg-purple-800 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-full text-sm my-5 px-5 py-2.5 text-center mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">login</button>
