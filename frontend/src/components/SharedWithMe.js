...
function SharedWithMe({setLoginModal,setShowLogin}) {
    ...
    const openFile=async(url,name)=>{
      ...
      const getSharedWithMe=async()=>{
        console.log("in getSharedWithMe");
        console.log(filesToShow)
        let document;
         try {
            document= await axios.get(`${window.location.origin}/api/shareWithMe`);
            setShowLogin(false);
            setLoading(false);
         } catch (error) {
            setShowLogin(true);
    ...
    document.data[0].receivedImage.map((item)=>{
        console.log("images");
        console.log(item);
    ...
    document.data[0].receivedPdf.map((item)=>{
        console.log("pdf")
        setTempFilesToShow((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
        setFilesToShow((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
      // setTempUploadedData((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
    })
    document.data[0].receivedSong.map((item)=>{
        console.log("song")
        setTempFilesToShow((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
        setFilesToShow((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
      // setTempUploadedData((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
    })
    document.data[0].receivedVideo.map((item)=>{
        console.log("video")
        setTempFilesToShow((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
        setFilesToShow((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
      // setTempUploadedData((prev)=>[item,...prev].sort((a,b)=>b.time-a.time ))
  ...
  return (
    ...
    {(loading)?<Skeleton/>:<></>}
        ...
        <MdOutlineDownload className='absolute h-8 w-8 text-slate-500 right-0 m-2 bottom-0 cursor-pointer' onClick={()=>openFile(item.fileData.link,item.fileData.name)}/>
        <RiDeleteBin6Line className='absolute h-6 w-6 text-slate-500 left-0 m-2 top-0 cursor-pointer'/>
        ...
        <div className='absolute h-[30%] w-full bottom-0 px-[3px]'>
          ...
          <p className='text-[13px] flex justify-between'><p>Shared by <strong>{item.senderName}</strong></p>   {new Date(parseInt(item.time)).toLocaleTimeString("en-US")}--{new Date(parseInt(item.time)).toLocaleDateString("en-GB")}</p>
